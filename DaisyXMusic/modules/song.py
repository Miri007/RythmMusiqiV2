...
def song(client, message):
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    m = message.reply("🔎 Mahnını axtarıram...")
...
async def ytmusic(client, message: Message):
    ...
    try:
        ...
        with youtube_dl.YoutubeDL(opts) as ytdl:
            ...
            if duration > DURATION_LIMIT:
                await pablo.edit(
                    f"❌ Uzun video!, {DURATION_LIMIT} dəqiqə icazə verilmir, təqdim olunan videoya icazə verilir {duration} saniyə"
                )
                is_downloading = False
                return
    ...
    await client.send_video(
        ...
        progress_args=(
            pablo,
            c_time,
            f"`Yükləyir {urlissed} YouTube Music-dən mahnı!`",
            file_stark,
        ),
    )
    await pablo.delete()
